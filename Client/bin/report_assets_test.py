...
BASE_DIR = os.path.dirname(os.getcwd())  # 将基础目录设置成为Client

sys.path.append(BASE_DIR)  # 将Client整个大包所在路径添加至解释器搜索路径，确保能够导入Client包中的模块
...
def update_test(data):
    ...
    print('正在将资产数据信息发往：   {}'.format(url))

    try:
        data_encode = parse.urlencode(data).encode()
        response = request.urlopen(url=url, data=data_encode, timeout=int(settings.Params['request_timeout']))
        print('*' * 10 + '正在发送数据' + '*' * 10)
        message = response.read().decode()
        print('*' * 10 + '结果：{}'.format(message) + '*' * 10)
    except Exception as e:
        message = '*' * 10 + '发送失败{}'.format(e) + '*' * 10
        print(message)
...
if __name__ == '__main__':
    windows_data = {
        ...
        "asset_type": "server",
        "cpu_count": 10,
        "cpu_model": "AIntel(R) Core(TM) i5-2300 CPU @ 2.80GHz",
        "cpu_core_count": 8,
        "ram": [
            {
                "slot": "A1",
        ...
        "model": "P67X-UD3R-B3",
        "wake_up_type": 6,
        "sn": "01426-23EM-8992662-21313213",
        "physical_disk_driver": [
            {
                "iface_type": "unknown",
                "slot": 0,
                "sn": "1830414130423230343234362020202020202020",
