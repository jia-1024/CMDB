...
class Asset(models.Model):
    ...
    admin = models.ForeignKey(User, null=True, blank=True, verbose_name='资产管理员', related_name='myadmin',
                              on_delete=models.SET_NULL)
...
class CPU(models.Model):
    ...
    def __str__(self):
        if not self.asset.name:
            return '自定义资产名：' + self.cpu_model
        if not self.cpu_model:
            return self.asset.name + "自定义CPU型号"
        if not all((self.asset.name, self.cpu_model)):
            return '自定义资产名:自定义CPU型号'
